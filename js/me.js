;window.Eager=window.Eager||{};Eager.siteId="Q1X4bFOTwg";Eager.generation="20160729214626-1872";Eager.installs=Eager.installs||{};;(function(){var prevEls={};Eager.createElement=function(options,prevEl){try{if(prevEl&&prevEl.parentNode){var replacedEl;if(prevEl.eagerElementId){replacedEl=prevEls[prevEl.eagerElementId];}
...
return false;}})();;Eager.installs["MFBjSiXCRsCL"]={appId:"HrJt4yHuuL4e",scope:{}};;Eager.installs["MFBjSiXCRsCL"].options={"behavior":{"outMode":"out"},"fewerParticles":false,"interaction":{"onClick":"push","onHover":"grab"},"particleColor":"000000","showAdvanced":false};;if(Eager.matchPage(Eager.installs['MFBjSiXCRsCL'].URLPatterns)){(function(Math){var trimLeft=/^\s+/,trimRight=/\s+$/,tinyCounter=0,mathRound=Math.round,mathMin=Math.min,mathMax=Math.max,mathRandom=Math.random;function tinycolor(color,opts){color=(color)?color:'';opts=opts||{};if(color instanceof tinycolor){return color;}
...
else{window.tinycolor=tinycolor;}})(Math);};if(Eager.matchPage(Eager.installs['MFBjSiXCRsCL'].URLPatterns)){var pJS=function(tag_id,params){var canvas_el=document.querySelector("#"+tag_id+" > .particles-js-canvas-el")
...
xhr.send()}};if(Eager.matchPage(Eager.installs['MFBjSiXCRsCL'].URLPatterns)){"use strict";(function(){if(!window.addEventListener)return;var _window=window;var tinycolor=_window.tinycolor;var CONTAINER_ID="eager-particles-js";var getComputedStyle=document.defaultView.getComputedStyle.bind(document.defaultView);var options=Eager.installs['MFBjSiXCRsCL'].options;var element=void 0;function getParticleColor(){var particleColor=void 0;if(options.particleColor){particleColor=tinycolor(options.particleColor);}else{var backgroundColor=options.backgroundColor||getComputedStyle(document.body).backgroundColor;var components=tinycolor(backgroundColor).toHsl();components.l=Math.abs((components.l+0.5)%1)+(1-components.s)*0.1;particleColor=tinycolor(components);}
...
window.Eager.installs['MFBjSiXCRsCL'].scope={setColors:function setColors(nextOptions){options=nextOptions;if(!window.pJSDom){updateElement();return;}
